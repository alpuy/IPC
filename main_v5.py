...
def draw_circle(event, x, y, flags, param):
    ...
    if event == cv2.EVENT_LBUTTONDBLCLK: # Check if the left button is double clicked
        cv2.circle(img, (x, y), 50, (255, 0, 0), -1) # Draw a blue circle on the image
...
def drag_circle(event, x, y, flags, param):
    ...
    if event == cv2.EVENT_LBUTTONDOWN: # Check if the left button is pressed down
        for i in range(len(l_copy)): # Loop through the list of points
            if np.linalg.norm(np.array([x, y]) - np.array(l_copy[i])) < 100: # Check if the mouse position is close to any point
        ...
        if dragging: # Check if the dragging flag is True
            ...
            for p in l_copy: # Loop through the list of points
                cv2.circle(img_copy, p, 50, (255, 0, 0), -1) # Draw a blue circle on the copied image
...
if len(l) == 4:
    ...
    dst = cv2.warpPerspective(img_unmod, M, dsize, borderMode=cv2.BORDER_CONSTANT, borderValue=(255, 255, 255)) # Apply the perspective transform to the image using OpenCV 
